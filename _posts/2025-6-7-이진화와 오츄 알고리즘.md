...
T를 구하는 과정을 생각해보면, T값을 중심으로 2개의 섹션을 나눠서 u는 각 섹션의 픽셀의 평균을, v는 각 섹션의 픽셀의 분산을 나타낸다, 그래서 그 분산의 값과 가중치의 값을 곱한걸 더한게 가장 작은, 가장 좋은 T를 최종 임계값으로 하고자 하는거다. 각 섹션의 픽셀들의 명암 분포가 균일할수록, 적절하게 T를 중간에 넣었다는 말이기 때문이다, 그래서 분포값을 고려하는거다.
...
`fn = v1 * q1 + v2 * q2`:  두 그룹의 분산을 가중 평균 낸 값(Within-Class Variance)이다. 이 값이 작을수록 "같은 그룹끼리 잘 뭉쳐있다"는 뜻이므로, 이 `fn`이 최소가 되는 지점을 찾아야 한다.
