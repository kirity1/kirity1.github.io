...
# 만약 라벨 없이 이미지 파일만 있는 경우, 모든 이미지를 읽어오게 됌.
...
def sample_batch(loader):
    ...
    #ImageFolder를 사용하면 배치마다 (이미지, 라벨) 튜플을 반환
    #여기서는 주로 시각화용으로 이미지 배치만 사용
...
def show_face_batch(batch, nrow=8):
    ...
    # normalize=True 옵션을 주면 픽셀 값을 [0, 1] 범위로 정규화
    grid_img = make_grid(batch, nrow=nrow, padding=2, normalize=True)
    
    plt.figure(figsize=(10, 10))
    # 텐서의 shape은 (C, H, W)이므로, (H, W, C)로 변경해 matplotlib에서 올바르게 표시되게 함.
...
# 만약 DataLoader가 (이미지, 레이블) 튜플을 반환한다면 images만 사용
